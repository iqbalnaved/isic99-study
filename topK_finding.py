...
def compute_top_k_similarities(query_features, example_img_features, k):
    ...
    for query_name, query_vec in query_features.items():
        similarities = [(name, float(1 - cosine(query_vec, example_img_features[name]))) # EDIT: added float() to solve TypeError: Object of type float32 is not JSON serializable
